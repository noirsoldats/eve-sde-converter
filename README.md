...
```bash
# Download the SQLite database (use the release tag, e.g., sde-2621031)
wget https://github.com/noirsoldats/eve-sde-converter/releases/download/sde-{version}/eve.db
...
**Files available:**
- `eve.db` - Full database with all SDE data (~200 MB)
- `eve-stripped.db` - Essential tables only (~50 MB)

#### MySQL

```bash
# Download the MySQL dump (use the release tag, e.g., sde-2621031)
wget https://github.com/noirsoldats/eve-sde-converter/releases/download/sde-{version}/eve-mysql.sql.gz
...
gunzip -c eve-mysql.sql.gz | mysql -u root -p evesde
...
```bash
# Download the PostgreSQL dump (use the release tag, e.g., sde-2621031)
wget https://github.com/noirsoldats/eve-sde-converter/releases/download/sde-{version}/eve-postgresql.sql.gz

# Create database
createdb evesde

# Import dump (decompresses on the fly)
gunzip -c eve-postgresql.sql.gz | psql evesde
...
```bash
# Download and extract the MSSQL dump (use the release tag, e.g., sde-2621031)
wget https://github.com/noirsoldats/eve-sde-converter/releases/download/sde-{version}/eve-mssql.sql.gz
gunzip eve-mssql.sql.gz
...
# Import dump
sqlcmd -S localhost -U sa -P YourPassword -d evesde -i eve-mssql.sql
